...
public class SetCodec<A> implements Codec<Set<A>> {
    ...
    public <T> DataResult<Pair<Set<A>, T>> decode(DynamicOps<T> ops, T input) {
        if (!(ops instanceof ExtendedOps)) {
            throw new UnsupportedOperationException("Only supported on ExtendedOps! Please use NbtOps, JanksonOps, or some other ExtendedOps implementation instead!");
        }
    ...
    public <T> DataResult<T> encode(Set<A> input, DynamicOps<T> ops, T prefix) {
        if (!(ops instanceof ExtendedOps)) {
            throw new UnsupportedOperationException("Only supported on ExtendedOps! Please use NbtOps or JanksonOps instead!");
        }
