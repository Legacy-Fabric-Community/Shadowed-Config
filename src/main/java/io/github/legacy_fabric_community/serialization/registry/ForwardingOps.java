...
public abstract class ForwardingOps<T> implements ExtendedOps<T> {
    protected final ExtendedOps<T> delegate;

    protected ForwardingOps(ExtendedOps<T> delegate) {
    ...
    @Override
    public SetBuilder<T> setBuilder() {
        return this.delegate.setBuilder();
    }
    ...
    private static class Impl<T> extends ForwardingOps<T> {
        private Impl(ExtendedOps<T> delegate) {
            super(delegate);
        }
    }
}