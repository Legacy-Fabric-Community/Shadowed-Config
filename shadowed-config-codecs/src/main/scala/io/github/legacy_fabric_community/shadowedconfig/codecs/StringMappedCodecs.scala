...
object StringMappedCodecs {
	...
	val BLOCK_ENTITY: Codec[Class[_$1]] forSome {type _$1 <: BlockEntity} = Codec.STRING.comapFlatMap((name: String) => Converters.toBlockEntity(name), (key: Class[_ <: BlockEntity]) => Converters.BE_CLASS_STRING_MAP.get(key))
	...
	private object Converters {
		...
		def toEntity(name: String): DataResult[Class[_ <: Entity]] = DataResult.success(Objects.requireNonNull(EntityTypeAccessor.getNameClassMap.get(name)))
