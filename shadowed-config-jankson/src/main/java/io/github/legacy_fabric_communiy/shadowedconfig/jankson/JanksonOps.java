...
public enum JanksonOps implements ExtendedOps<JsonElement> {
    INSTANCE;

    public static final Codec<JsonObject> JSON_OBJECT_CODEC = Codec.PASSTHROUGH.comapFlatMap((dynamic) -> {
        JsonElement element = dynamic.convert(INSTANCE).getValue();
        return element instanceof JsonObject ? DataResult.success((JsonObject) element) : DataResult.error("Not a Json Object: " + element);
    }, (jsonObject) -> new Dynamic<>(INSTANCE, jsonObject));
    ...
    public DataResult<Boolean> getBooleanValue(JsonElement input) {
        if (input instanceof JsonPrimitive) {
            if (((JsonPrimitive) input).getValue() instanceof Boolean) {
                ...
                return ExtendedOps.super.getBooleanValue(input);
            }
        }
